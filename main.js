  ...
  try{
    ...
    (data.features||[]).forEach(f=>{
      if(f.geometry.type==='Polygon'){
        ...
        const marker = L.marker([f.geometry.coordinates[1],f.geometry.coordinates[0]],{icon,rotationAngle:f.properties.angulo||0});
...
function renderNews(){
  ...
  noticias.forEach(n=>{
    ...
    const img=document.createElement('img'); img.src=n.imgBase64||'';
...
// --- Inicialización ---
(async()=>{await loadMap(); await loadNews();})();
